...
- **Gap:** Tests validate outcomes but don't assert _how_ those outcomes are achieved (call counts, short-circuit behavior, error paths)
...
**In robustness.spec.ts:** 3. `delegate()` with symbol property keys 4. Strategy metadata lookup with symbol keys 5. Delegation to undefined/missing property
