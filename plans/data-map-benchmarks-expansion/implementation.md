...
- [ ] Create `packages/data-map/benchmarks/src/signals-comparative.bench.ts`:

```ts
import { bench, describe } from 'vitest';

import { SIGNAL_ADAPTERS } from './adapters/index.js';
...
- [ ] Create `packages/data-map/benchmarks/src/signals-scale.bench.ts`:
...
const COUNTS = [1, 100, 10_000] as const;
	...
	for (const adapter of SIGNAL_ADAPTERS) {
		for (const n of COUNTS) {
			bench(
				...
				() => {
					const signals = [] as ReturnType<
						typeof adapter.createSignal<number>
					>[];
					for (let i = 0; i < n; i++) signals.push(adapter.createSignal(i));
					if (signals.length !== n) throw new Error('bad');
				},
			);
		}
	}
...
- [ ] `pnpm --filter @data-map/benchmarks exec vitest bench src/signals-comparative.bench.ts`
- [ ] `pnpm --filter @data-map/benchmarks exec vitest bench src/signals-scale.bench.ts`
