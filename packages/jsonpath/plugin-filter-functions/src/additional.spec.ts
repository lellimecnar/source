...
describe('@jsonpath/plugin-filter-functions (additional)', () => {
	it('exports stable plugin id', () => {
		expect(plugin.meta.id).toBe('@jsonpath/plugin-filter-functions');
	});

	it('declares the expected capability', () => {
		expect(plugin.meta.capabilities).toContain('filter:rfc9535:functions');
	});
