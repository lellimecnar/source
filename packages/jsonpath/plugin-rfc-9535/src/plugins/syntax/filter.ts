	...
	createPlugin({
		...
		setup: ({ engine }) => {
			engine.evaluators.registerSegment(
				...
				(
					inputs,
					segment: SegmentNode & { expression: any },
					evaluators,
					ctx: EvalContext,
				) => {
					const evalExpression = evaluators.getExpression(
						segment.expression.kind,
					);
					if (!evalExpression) {
						throw new JsonPathError({
							code: JsonPathErrorCodes.Evaluation,
							message: `No evaluator registered for expression kind: ${segment.expression.kind}`,
						});
					}
					return inputs.filter((n) =>
						evalExpression(segment.expression, { ...ctx, current: n }),
					);
				},
			);
		},
