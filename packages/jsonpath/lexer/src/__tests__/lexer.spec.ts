...
describe('Lexer', () => {
	...
	it('should tokenize boolean and null literals', () => {
		...
		expect(tokens.filter(t => [TokenType.TRUE, TokenType.FALSE, TokenType.NULL].includes(t.type)).map(t => t.value)).toEqual([
			true, false, null
		]);
	});
	...
	it('should tokenize comparison operators', () => {
		...
		const ops = tokens.filter(t => [TokenType.EQ, TokenType.NE, TokenType.AND].includes(t.type));
	...
	it('should perform well (benchmark)', () => {
		const query = '$.store.book[?(@.price < 10 && @.category == "fiction")].title';
